...
(function () {
  ...
  var PRESETS = [
    ...
    {
      id: 'preset_dracolich', name: 'Dracolich', emoji: '🐲', rarity: 'epic',
      type: 'Non Morto', size: 'Enorme', alignment: 'Legale Malvagio',
      ac: 20, hp: 225, hpCur: 225, hpTemp: 0, hpDice: '18d12+108', init: 12,
      speed: '12 m, Scavare 9 m, Volare 24 m', cr: '17', xp: '',
      str: 25, dex: 10, con: 23, intl: 19, wis: 15, cha: 21,
      savesOverride: { str: '', dex: '6', con: '', intl: '', wis: '8', cha: '' },
      skillOverrides: { percezione: '14', furtivita: '6' }, passivePerception: 24,
      senses: [{ type: 'Vista Cieca', value: 18, unit: 'm' }, { type: 'Scurovisione', value: 36, unit: 'm' }], languages: ['Comune', 'Draconico'],
      dmgResist: [], dmgImmune: ['Necrotico', 'Veleno'], dmgVulner: [],
      condImmune: ['Ammaliato', 'Esausto', 'Spaventato', 'Paralizzato', 'Avvelenato'],
      traits: [
        { name: 'Resistenza Leggendaria', desc: '(3/Giorno, oppure 4/Giorno nel Covo)\nSe il dracolich fallisce un tiro salvezza, può scegliere di superarlo invece.' },
        { name: 'Soppressione della Vita', desc: 'Le creature entro 18 metri dal dracolich non possono recuperare punti ferita.' },
        { name: 'Resistenza Magica', desc: 'Il dracolich ha vantaggio ai tiri salvezza contro incantesimi e altri effetti magici.' },
        { name: 'Gemma dell\'Anima', desc: 'Il dracolich possiede una gemma magica. Se il dracolich viene distrutto mentre la gemma si trova sullo stesso piano di esistenza, ottiene un nuovo corpo in 1d20 giorni, recuperando tutti i suoi punti ferita e comparendo entro 1,5 metri dalla gemma.\nLa gemma è un oggetto Minuscolo con CA 20, 50 PF e immunità ai danni necrotici, da veleno e psichici. Recupera tutti i suoi punti ferita alla fine di ogni turno, ma si riduce in polvere se portata a 0 PF.\nSe la gemma viene distrutta, il dracolich può crearne una nuova completando un rituale di 8 ore con una gemma del valore di almeno 1.000 mo e spendendo altre 5.000 mo, consumate dal rituale.' }
      ],
      actions: [
        { name: 'Multiattacco', desc: 'Il dracolich effettua tre attacchi con Artigliare. Può sostituire uno degli attacchi con un uso di Lancio di Incantesimi per lanciare Raggio di Infermità (versione di 2° livello).' },
        { name: 'Artigliare', desc: 'Attacco con arma da mischia: +13 a colpire, portata 3 metri.\nColpito: 18 (2d10 + 7) danni taglienti più 4 (1d8) danni necrotici.', atkHit: '+13', atkDmgs: [{ f: '2d10+7', t: 'taglienti' }, { f: '1d8', t: 'necrotici' }] },
        { name: 'Soffio Necrotico (Ricarica 5–6)', desc: 'Tiro Salvezza su Costituzione: CD 20, ogni creatura in un cono di 18 metri.\nFallimento: 52 (8d12) danni necrotici.\nSuccesso: metà danni.', atkDmgs: [{ f: '8d12', t: 'necrotici' }] },
        { name: 'Lancio di Incantesimi', desc: 'Il dracolich lancia uno dei seguenti incantesimi senza componenti materiali, usando il Carisma come caratteristica da incantatore (CD 19, +11 a colpire con gli incantesimi).\n\nA volontà: Individuazione del Magico, Raggio di Infermità (versione di 2° livello).\n\n1/Giorno ciascuno: Creare Non Morti (versione di 8° livello), Dito della Morte.' }
      ],
      bonusActions: [],
      reactions: [],
      legendaryActions: [
        { name: 'Usi (3 per round, 4 nel Covo)', desc: 'Subito dopo il turno di un\'altra creatura, il dracolich può spendere un uso per effettuare una delle azioni seguenti. Recupera tutti gli usi spesi all\'inizio di ciascun suo turno.' },
        { name: 'Balzo', desc: 'Il dracolich si muove fino a metà della propria Velocità ed effettua un attacco con Artigliare.', atkHit: '+13', atkDmgs: [{ f: '2d10+7', t: 'taglienti' }, { f: '1d8', t: 'necrotici' }] },
        { name: 'Raggio Debilitante', desc: 'Il dracolich usa Lancio di Incantesimi per lanciare Raggio di Infermità (versione di 2° livello). Non può usare di nuovo questa azione fino all\'inizio del suo turno successivo.' },
        { name: 'Presenza Terrificante', desc: 'Tiro Salvezza su Saggezza: CD 19, ogni creatura in un\'emanazione di 9 metri originata dal dracolich.\nFallimento: 11 (2d10) danni psichici e il bersaglio ottiene la condizione Spaventato fino alla fine del suo turno successivo.\nFallimento o Successo: il dracolich non può usare di nuovo questa azione fino all\'inizio del suo turno successivo.', atkDmgs: [{ f: '2d10', t: 'psichici' }] }
      ],
      drop: [{ name: 'Qualsiasi', desc: 'Il bottino accumulato in vita e in non morte, spesso con la gemma dell\'anima nascosta tra i tesori.' }],
      notes: 'Tiranno Draconico Rinato\n\nHabitat: Qualsiasi\nTesoro: Qualsiasi\n\n(Affrontato nel proprio covo, il dracolich vale 20.000 PE.)\n\nI draghi più malvagi cercano di sfuggire alla morsa della morte, impiegando segreti senza età e magie blasfeme per diventare orrori chiamati dracolich. Questi draghi immorti legano il proprio spirito a gemme e animano magicamente i loro cadaveri in decomposizione. Trasformatisi infine in orrori scheletrici, i dracolich continuano i complotti secolari perseguiti in vita, cercano vendetta contro coloro che li hanno condotti alla rovina e perseguono obiettivi feroci che non poterono soddisfare quando erano vivi.\n\nI dracolich combinano la corrotta immortalità dei non morti con il leggendario potere dei draghi. Un dracolich conserva un\'arma a soffio, ma questa si manifesta come una gelida esplosione necrotica. Ammalano gradualmente le terre attorno ai loro covi e attirano sinistri seguaci, di solito altri non morti o cultisti. I draghi viventi di ogni tipo odiano i dracolich e cercano di distruggerli, considerandoli perversioni della magia draconica.\n\nUna volta creato, un dracolich sceglie una gemma che diventa l\'ancora del suo spirito. Finché si trova sullo stesso piano di esistenza della sua gemma dell\'anima, può sopravvivere alla distruzione del proprio corpo: lo spirito si ritira nella gemma e il mostro potrebbe un giorno riottenere la sua terrificante forma.\n\nContenitori della Gemma dell\'Anima (1d10) — la gemma dell\'anima è nascosta in...\n1 — Il tesoro di un altro drago.\n2 — Il corpo di un servitore o di un antenato.\n3 — Il nucleo del tesoro fuso di un dracolich.\n4 — Un uovo di drago corrotto.\n5 — Un corno di drago preso come trofeo da un eroe.\n6 — Un tesoro reale o religioso di una nazione.\n7 — Un potente oggetto magico.\n8 — Una fonte di meraviglie magiche, come un albero gigantesco o una pozza mistica.\n9 — La cassaforte di un arcidiavolo, di una divinità malvagia o di un altro antagonista extraplanare.\n10 — L\'arma che ha ucciso il dracolich.\n\n— Covo del Dracolich —\nUn dracolich si annida in una versione corrotta del covo che possedeva in vita. La regione entro 1 miglio dal covo è deformata dalla sua presenza, producendo i seguenti effetti regionali.\n\nNebbia Prosciugante: l\'area entro 1 miglio dal covo è Leggermente Oscurata da una nebbia pallida. Ogni creatura diversa dal dracolich e dai suoi alleati che completa un Riposo Lungo nell\'area deve superare un Tiro Salvezza su Costituzione CD 15 oppure subire 1 livello di Affaticamento.\n\nProsciugamento dell\'Anima: le creature entro 1 miglio dal covo hanno Svantaggio ai Tiri Salvezza contro la Morte.\n\nSe il dracolich viene distrutto o sposta altrove il covo, questi effetti terminano immediatamente; riprendono se ottiene un nuovo corpo (vedi il tratto Gemma dell\'Anima).\n\n«E nulla rimarrà se non troni infranti senza sovrani. Ma i draghi morti governeranno il mondo intero...»\n— Sammaster il Caduto, traduzione della Cronaca degli Anni Futuri'
    },
    {
      id: 'preset_drago_tartaruga', name: 'Drago Tartaruga', emoji: '🐢', rarity: 'epic',
      type: 'Drago', size: 'Mastodontica', alignment: 'Neutrale',
      ac: 20, hp: 356, hpCur: 356, hpTemp: 0, hpDice: '23d20+115', init: 6,
      speed: '6 m, Nuotare 15 m', cr: '17', xp: '',
      str: 25, dex: 10, con: 20, intl: 10, wis: 12, cha: 12,
      savesOverride: { str: '', dex: '', con: '11', intl: '', wis: '7', cha: '' },
      skillOverrides: {}, passivePerception: 11,
      senses: [{ type: 'Scurovisione', value: 36, unit: 'm' }], languages: ['Draconico', 'Primordiale (Acquan)'],
      dmgResist: ['Fuoco'], dmgImmune: [], dmgVulner: [],
      condImmune: [],
      traits: [
        { name: 'Anfibio', desc: 'Il drago può respirare sia aria sia acqua.' }
      ],
      actions: [
        { name: 'Multiattacco', desc: 'Il drago effettua tre attacchi con Morso. Può sostituire uno di questi attacchi con un attacco con Coda.' },
        { name: 'Morso', desc: 'Attacco con arma da mischia: +13 a colpire, portata 4,5 metri.\nColpito: 23 (3d10 + 7) danni perforanti più 7 (2d6) danni da fuoco.\nEssere sott\'acqua non conferisce resistenza a questi danni da fuoco.', atkHit: '+13', atkDmgs: [{ f: '3d10+7', t: 'perforanti' }, { f: '2d6', t: 'fuoco' }] },
        { name: 'Coda', desc: 'Attacco con arma da mischia: +13 a colpire, portata 4,5 metri.\nColpito: 18 (2d10 + 7) danni contundenti.\nSe il bersaglio è una creatura di taglia Enorme o inferiore, ottiene la condizione Prono.', atkHit: '+13', atkDmgs: [{ f: '2d10+7', t: 'contundenti' }] },
        { name: 'Soffio di Vapore (Ricarica 5–6)', desc: 'Tiro Salvezza su Costituzione: CD 19, ogni creatura in un cono di 18 metri.\nFallimento: 56 (16d6) danni da fuoco.\nSuccesso: metà danni.\nEssere sott\'acqua non conferisce resistenza a questi danni da fuoco.', atkDmgs: [{ f: '16d6', t: 'fuoco' }] }
      ],
      bonusActions: [], reactions: [], legendaryActions: [],
    ...
    {
      id: 'preset_cataclisma_elementale', name: 'Cataclisma Elementale', emoji: '🌋', rarity: 'legendary',
      type: 'Elementale', size: 'Mastodontica', alignment: 'Caotico Neutrale',
      ac: 20, hp: 370, hpCur: 370, hpTemp: 0, hpDice: '20d20+160', init: 18,
      speed: '18 m, Scavare 18 m, Volare 24 m (fluttuare), Nuotare 24 m', cr: '22', xp: '',
      str: 26, dex: 19, con: 27, intl: 9, wis: 14, cha: 9,
      savesOverride: { str: '', dex: '11', con: '15', intl: '', wis: '9', cha: '6' },
      skillOverrides: {}, passivePerception: 12,
      senses: [{ type: 'Vista del Vero', value: 45, unit: 'm' }], languages: ['Primordiale'],
      dmgResist: [], dmgImmune: ['Acido', 'Freddo', 'Fuoco', 'Fulmine', 'Veleno', 'Tuono'], dmgVulner: [],
      condImmune: ['Accecato', 'Ammaliato', 'Assordato', 'Esausto', 'Spaventato', 'Afferrato', 'Paralizzato', 'Pietrificato', 'Avvelenato', 'Prono', 'Trattenuto', 'Stordito', 'Privo di Sensi'],
      traits: [
        { name: 'Attraversare la Terra', desc: 'Il cataclisma può scavare attraverso terra e pietra non lavorate e non magiche. Mentre lo fa, non disturba il materiale che attraversa.' },
        { name: 'Resistenza Leggendaria (4/Giorno)', desc: 'Se il cataclisma fallisce un tiro salvezza, può scegliere di superarlo invece.' },
        { name: 'Mostro d\'Assedio', desc: 'Il cataclisma infligge danni doppi a oggetti e strutture.' }
      ],
      actions: [
        { name: 'Multiattacco', desc: 'Il cataclisma effettua due attacchi con Esplosione Elementale.' },
        { name: 'Esplosione Elementale', desc: 'Attacco con arma da mischia o a distanza: +15 a colpire, portata 9 metri o gittata 45 metri.\nColpito: 25 (5d6 + 8) danni di un tipo a scelta del cataclisma: acido, freddo, fuoco, fulmine o tuono.', atkHit: '+15', atkDmgs: [{ f: '5d6+8', t: 'elementali' }] },
        { name: 'Evento Cataclismico (Ricarica 4–6)', desc: 'Il cataclisma genera casualmente uno dei seguenti effetti (tira 1d4).\n\n1. Fiamme Avvinghianti — TS Destrezza CD 23, ogni creatura in una sfera di raggio 18 metri centrata su un punto entro 45 metri. Fallimento: 45 (13d6) danni da fuoco. Successo: metà danni. In entrambi i casi il bersaglio prende fuoco.\n\n2. Ondate Gelide — TS Forza CD 23, ogni creatura in un cono di 27 metri. Fallimento: 22 (5d8) danni contundenti più 22 (5d8) danni da freddo e Prono. Successo: metà danni. In entrambi i casi la velocità del bersaglio è ridotta a 0 fino alla fine del suo turno successivo.\n\n3. Tempesta Furiosa — una nube tempestosa riempie una sfera di raggio 18 metri centrata su un punto entro 45 metri e permane per 1 minuto o finché il cataclisma non riusa Evento Cataclismico. Le creature interamente nella nube sono Accecate e Assordate e non possono lanciare incantesimi con componenti verbali. TS Destrezza CD 23 per ogni creatura che entra nella nube per la prima volta in un turno o vi inizia il turno. Fallimento: 18 (4d8) danni da fulmine più 18 (4d8) danni da tuono. Successo: metà danni.\n\n4. Terra Inghiottente — TS Forza CD 23, ogni creatura in un cubo di 27 metri di lato originato da un punto sul terreno entro 45 metri. Fallimento: 18 (4d8) danni contundenti più 18 (4d8) danni da acido; il bersaglio ottiene Prono e viene sepolto sotto le macerie (Trattenuto, Copertura Totale, sta soffocando). Come azione, la creatura sepolta o un\'altra entro 1,5 metri può superare una prova di Forza (Atletica) CD 18 per liberarla. Successo: metà danni.' },
        { name: 'Controllare Tempo Atmosferico', desc: 'Il cataclisma lancia Controllare Tempo Atmosferico senza componenti materiali, usando la Costituzione come caratteristica da incantatore.' }
      ],
      bonusActions: [],
      reactions: [],
      legendaryActions: [
        { name: 'Usi (3 per round)', desc: 'Immediatamente dopo il turno di un\'altra creatura, il cataclisma può spendere un uso per compiere una delle azioni seguenti. Recupera tutti gli usi spesi all\'inizio del proprio turno.' },
        { name: 'Eruzione', desc: 'Il cataclisma effettua un attacco con Esplosione Elementale.', atkHit: '+15', atkDmgs: [{ f: '5d6+8', t: 'elementali' }] },
        { name: 'Movimento Rombante', desc: 'Il cataclisma si muove fino alla propria Velocità, Velocità di Volo o di Nuoto senza provocare attacchi di opportunità.\nOgni creatura entro 1,5 metri dal cataclisma mentre esso si muove deve effettuare un Tiro Salvezza su Costituzione CD 23.\nFallimento: il bersaglio ottiene la condizione Prono.\nFallimento o Successo: il cataclisma non può usare di nuovo questa azione fino all\'inizio del suo turno successivo.' }
      ],
      drop: [],
    ...
    {
      id: 'preset_iota_empireo', name: 'Iota Empireo', emoji: '✨', rarity: 'uncommon',
      type: 'Celestiale', size: 'Media', alignment: 'Neutrale',
      ...
      notes: 'Scioni degli Dei\n\nHabitat: Qualsiasi\nTesoro: Reliquie\n\n(Celestiale o Immondo a seconda del genitore divino: celestiale per le divinità dei Piani Superiori, immondo per quelle dei Piani Inferiori.)\n\nGli empirei sono la progenie delle divinità. Pur non essendo essi stessi dèi, possiedono influenza e poteri divini legati ai loro genitori.\n\nGli iota empirei sono i pensieri, i ricordi o le attenzioni dirette delle divinità, personificati e dotati di una certa autonomia. Simili a glifi fluttuanti, bagliori di colore, icone della divinità o forme ancora più strane, questi esseri sono messaggeri degli dèi e aiutanti dei loro servitori. Gli iota possiedono conoscenze e personalità proprie, sebbene possano conoscere ben poco oltre ai reami e ai desideri dei loro creatori.\n\nInfluenze dell\'Empireo (1d6) — l\'iota possiede caratteristiche che sono...\n1 — Equilibrate, naturalistiche o che ricordano occhi che osservano.\n2 — Colorate, ombrose o fluide, oppure che variano a seconda dell\'osservatore.\n3 — Confortanti e gentili, o che richiamano piacevoli ricordi.\n4 — Parti scollegate, pensieri visibili o forme erratiche.\n5 — Simili a macchine, stoiche, simmetriche o evocative del giudizio.\n6 — Macabre, minacciose o mostruose, oppure che incarnano le paure dell\'osservatore.\n\n«Un giorno potrei essere il dio delle tempeste: guidare innumerevoli seguaci, rispondere alle preghiere, cambiare mondi interi. Ma fino ad allora farò ciò che mi pare.»\n— Kopoha, progenie delle tempeste'
    },
    {
      id: 'preset_empireo', name: 'Empireo', emoji: '🌟', rarity: 'legendary',
      type: 'Celestiale', size: 'Enorme', alignment: 'Neutrale',
      ac: 22, hp: 346, hpCur: 346, hpTemp: 0, hpDice: '21d12+210', init: 19,
      speed: '15 m, Volare 15 m (fluttuare), Nuotare 15 m', cr: '23', xp: '',
      str: 30, dex: 21, con: 30, intl: 21, wis: 22, cha: 27,
      savesOverride: { str: '', dex: '', con: '', intl: '', wis: '', cha: '' },
      skillOverrides: { intuizione: '13', percezione: '13' }, passivePerception: 23,
      senses: [{ type: 'Vista del Vero', value: 36, unit: 'm' }], languages: ['Tutti'],
      dmgResist: ['Contundente', 'Perforante', 'Tagliente'], dmgImmune: ['Necrotico', 'Radioso'], dmgVulner: [],
      condImmune: [],
      traits: [
        { name: 'Resistenza Leggendaria (4/Giorno)', desc: 'Se l\'empireo fallisce un tiro salvezza, può scegliere di superarlo invece.' },
        { name: 'Resistenza Magica', desc: 'L\'empireo ha vantaggio ai tiri salvezza contro incantesimi e altri effetti magici.' }
      ],
      actions: [
        { name: 'Multiattacco', desc: 'L\'empireo effettua due attacchi usando Arma Sacra o Raggio Divino, in qualsiasi combinazione.' },
        { name: 'Arma Sacra', desc: 'Attacco con arma da mischia: +17 a colpire, portata 3 metri.\nColpito: 31 (6d6 + 10) danni da forza e il bersaglio è Stordito fino all\'inizio del turno successivo dell\'empireo. Il bersaglio può scegliere di non essere stordito; in tal caso subisce invece 21 danni da forza aggiuntivi che ignorano resistenze e immunità.', atkHit: '+17', atkDmgs: [{ f: '6d6+10', t: 'forza' }] },
        { name: 'Raggio Divino', desc: 'Attacco con arma a distanza: +15 a colpire, gittata 180 metri.\nColpito: 35 (6d8 + 8) danni necrotici oppure radiosi (a scelta dell\'empireo).', atkHit: '+15', atkDmgs: [{ f: '6d8+8', t: 'necrotici o radiosi' }] },
        { name: 'Lancio di Incantesimi', desc: 'L\'empireo lancia uno dei seguenti incantesimi senza componenti materiali, usando il Carisma come caratteristica da incantatore (CD 23).\n\nA volontà: Calmare Emozioni, Ristorare Superiore, Passare Senza Tracce, Respirare Sott’Acqua.\n\n1/Giorno ciascuno: Comunione, Dissolvi il Bene e il Male, Spostamento Planare.' }
      ],
      bonusActions: [],
      reactions: [],
      legendaryActions: [
        { name: 'Usi (3 per round)', desc: 'Immediatamente dopo il turno di un\'altra creatura, l\'empireo può spendere un uso per compiere una delle azioni seguenti. Recupera tutti gli usi spesi all\'inizio di ciascun proprio turno.' },
        { name: 'Rinvigorire', desc: 'L\'empireo ottiene 10 punti ferita temporanei; lui e ogni alleato entro 9 metri ottengono vantaggio alle prove di d20 fino alla fine del turno successivo dell\'empireo. Non può usare di nuovo questa azione prima dell\'inizio del suo turno successivo.' },
        { name: 'Onda d\'Urto della Gloria', desc: 'Tiro Salvezza su Costituzione: CD 23, ogni creatura entro un\'emanazione di 9 metri originata dall\'empireo.\nFallimento: 27 (6d8) danni da forza e la creatura ottiene la condizione Prono.\nSuccesso: metà danni.\nFallimento o Successo: l\'empireo non può usare di nuovo questa azione fino all\'inizio del suo turno successivo.', atkDmgs: [{ f: '6d8', t: 'forza' }] },
        { name: 'Punire', desc: 'L\'empireo effettua un attacco con Raggio Divino.', atkHit: '+15', atkDmgs: [{ f: '6d8+8', t: 'necrotici o radiosi' }] }
      ],
    ...
    {
      id: 'preset_gargoyle', name: 'Gargoyle', emoji: '🏰', rarity: 'uncommon',
      ...
      traits: [
        { name: 'Attacco in Picchiata', desc: 'Il gargoyle non provoca attacchi di opportunità quando vola fuori dalla portata di un nemico.' }
      ],
      actions: [
        { name: 'Multiattacco', desc: 'Il gargoyle effettua due attacchi con Artiglio.' },
        { name: 'Artiglio', desc: 'Attacco con arma da mischia: +4 a colpire, portata 1,5 metri.\nColpito: 7 (2d4 + 2) danni taglienti.', atkHit: '+4', atkDmgs: [{ f: '2d4+2', t: 'taglienti' }] }
      ],
      bonusActions: [], reactions: [], legendaryActions: [],
      drop: [{ name: 'Qualsiasi', desc: 'Tesori accumulati nei covi elevati e oggetti rubati alle vittime delle imboscate.' }],
      notes: 'Sentinella Scolpita Nascosta in Piena Vista\n\nHabitat: Sottosuolo, Urbano\nTesoro: Qualsiasi\n\nI gargoyle sono sculture abitate da spiriti elementali. Ali e magia consentono ai loro pesanti corpi di pietra di volare, e spesso si appollaiano dove possono confondersi tra architetture elaborate, formazioni rocciose o semplici statue.\n\nServono generalmente gli incantatori che li hanno evocati nei loro corpi di pietra, ma se lasciati a sé stessi potrebbero dedicarsi a scherzi crudeli e rubare tesori da accumulare nei loro covi elevati.\n\nSculture di Gargoyle (1d6) — il gargoyle è scolpito per assomigliare a...\n1 — Un cherubino dai lineamenti eternamente sorridenti.\n2 — Una figura rozzamente scolpita o modellata naturalmente.\n3 — Una figura danneggiata o deturpata da parti non corrispondenti.\n4 — Un essere simile a un drago con scaglie di pietra levigata.\n5 — Un immondo dall\'aspetto gotico con corna e coda.\n6 — Un elemento utile, come un podio decorato o una colonna.\n\nAgguati: privi di necessità biologiche e dotati di pazienza soprannaturale, i gargoyle possono rimanere immobili per mesi, rivelandosi solo quando le condizioni sono perfette per attaccare.\n\nMimetizzazione del Gargoyle (1d8) — il gargoyle si nasconde tra...\n1 — Nodi e cortecce di un albero gigantesco.\n2 — Monumenti in un cimitero o memoriale.\n3 — Affioramenti rocciosi su una scogliera o una formazione di pietra.\n4 — Le vittime pietrificate di un basilisco o di una medusa.\n5 — Bassorilievi su un portale o un muro scolpito.\n6 — Macerie in una rovina o in una discarica.\n7 — Stalattiti o ghiaccioli sul soffitto di una caverna.\n8 — Statue su un castello, una magione o un tempio.\n\n«Dove il male attraversa il Piano Elementale della Terra, macchia la roccia e corrompe il suolo. La malizia svanisce tra gli altri elementi, ma nell\'oscuro sottosuolo assume forma e si trasforma in incubi.»\n— Tiv, Studioso dei Piani Elementali'
    },
    {
      id: 'preset_cubo_gelatinoso', name: 'Cubo Gelatinoso', emoji: '🧊', rarity: 'uncommon',
      type: 'Melma', size: 'Grande', alignment: 'Senza Allineamento',
      ac: 6, hp: 63, hpCur: 63, hpTemp: 0, hpDice: '6d10+30', init: -4,
      speed: '4,5 m', cr: '2', xp: '',
      str: 14, dex: 3, con: 20, intl: 1, wis: 6, cha: 1,
      savesOverride: { str: '', dex: '', con: '', intl: '', wis: '', cha: '' },
      skillOverrides: {}, passivePerception: 8,
      senses: [{ type: 'Vista Cieca', value: 18, unit: 'm' }], languages: [],
      dmgResist: [], dmgImmune: ['Acido'], dmgVulner: [],
      condImmune: ['Accecato', 'Ammaliato', 'Assordato', 'Esausto', 'Spaventato', 'Prono'],
      traits: [
        { name: 'Cubo di Melma', desc: 'Il cubo occupa completamente il proprio spazio ed è trasparente. Altre creature possono entrare in quello spazio, ma chi lo fa è soggetto all\'azione Ingoiare del cubo e ha svantaggio al tiro salvezza.\nLe creature all\'interno beneficiano di copertura totale; il cubo può contenere al massimo una creatura Grande oppure fino a quattro creature Medie o Piccole alla volta.\nCome azione, una creatura entro 1,5 metri può tentare di estrarre una creatura o un oggetto dal suo interno con una prova di Forza (Atletica) CD 12. Chi effettua il tentativo subisce 10 (3d6) danni da acido.' },
        { name: 'Trasparente', desc: 'Anche quando è perfettamente visibile, una creatura deve superare una prova di Saggezza (Percezione) CD 15 per notare il cubo, a meno che non lo abbia già visto muoversi o agire in altro modo.' }
      ],
      actions: [
        { name: 'Pseudopode', desc: 'Attacco con arma da mischia: +4 a colpire, portata 1,5 metri.\nColpito: 12 (3d6 + 2) danni da acido.', atkHit: '+4', atkDmgs: [{ f: '3d6+2', t: 'acido' }] },
        { name: 'Ingoiare', desc: 'Il cubo si muove fino alla sua Velocità senza provocare attacchi di opportunità e può attraversare gli spazi occupati da creature di taglia Grande o inferiore, se ha spazio sufficiente al proprio interno per contenerle (vedi Cubo di Melma).\nTiro Salvezza su Destrezza CD 12, per ogni creatura nel cui spazio il cubo entra per la prima volta durante questo movimento.\nFallimento: 10 (3d6) danni da acido e il bersaglio viene inglobato.\nUna creatura inglobata è soffocata, non può lanciare incantesimi con componenti verbali, ottiene la condizione Trattenuto e subisce 10 (3d6) danni da acido all\'inizio di ogni turno del cubo. Quando il cubo si muove, la creatura inglobata si sposta con esso.\nUna creatura inglobata può tentare di liberarsi usando un\'azione per effettuare una prova di Forza (Atletica) CD 12. Successo: fugge dal cubo nello spazio non occupato più vicino. Fallimento: metà danni e si sposta in uno spazio non occupato entro 1,5 metri dal cubo. Se non esiste uno spazio non occupato disponibile, il bersaglio fallisce invece il tiro salvezza.', atkDmgs: [{ f: '3d6', t: 'acido' }] }
      ],
      bonusActions: [], reactions: [], legendaryActions: [],
    ...
    {
      id: 'preset_gnoll_demoniaco', name: 'Gnoll Demoniaco', emoji: '🌑', rarity: 'rare',
      type: 'Immondo', size: 'Media', alignment: 'Caotico Malvagio',
      ac: 16, hp: 135, hpCur: 135, hpTemp: 0, hpDice: '18d8+54', init: 4,
      speed: '9 m', cr: '8', xp: '',
    ...
    {
      id: 'preset_goblin_scagnozzo', name: 'Scagnozzo Goblin', emoji: '🃏', rarity: 'common',
    ...
    {
      id: 'preset_goblin_sciamano', name: 'Sciamano Goblin', emoji: '🪬', rarity: 'uncommon',
      type: 'Fata', size: 'Piccola', alignment: 'Caotico Neutrale',
  ...
  var css =
    ...
    '.best__corner{position:absolute;top:4px;font-family:var(--mono);font-size:.56rem;font-weight:600;background:rgba(0,0,0,.5);padding:2px 5px;border-radius:5px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;z-index:2}' +
